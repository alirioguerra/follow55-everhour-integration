...
export class TimesheetViewProvider implements vscode.WebviewViewProvider {
  ...
  private async stopTimer() {
    ...
    try {
      ...
      if (response.ok) {
        // Calcular o tempo decorrido ANTES de limpar o timer
        const elapsedMinutes = this._currentTimer.startTime 
        ? Math.max(0, Math.floor((Date.now() - this._currentTimer.startTime) / 60000))
        : 0;
        
        const taskName = this.getTaskName(this._currentTimer.taskId);
        
        // Limpar o timer DEPOIS de calcular o tempo
        this._currentTimer = {};
        this.updateView();
  ...
  private _getHtmlForWebview(webview: vscode.Webview): string {
    ...
    ? this._tasks.filter(task => task.name.toLowerCase().includes(searchTerm))
    : this._tasks;
